...
itemRouter.post('/insereItem', async (req, res) => {
    ...
    const sql = "INSERT INTO Item (valor, ID_Etapa) VALUES (?, ?)";

    // The SQL command is executed, and the response is stored.
    const response = await query(sql, [req.body.valor, req.body.ID_Etapa]);
...
itemRouter.post('/removeItem', async (req, res) => {
    ...
    // The SQL command is executed, and the response is stored.
    const response = await query(sql, [req.body.ID_Item]);
